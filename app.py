...
with tab_perf:
    st.subheader("Regression metrics (on 173 unseen designs)")
    M = bundle.get("metrics", {})
    # headline numbers
    c = st.columns(4)
    c[0].metric("R²", M.get("R2", "?"))
    c[1].metric("Adjusted R²", M.get("Adjusted_R2", "?"))
    c[2].metric("Within 3×", f'{M.get("within_3x_pct","?")}%')
    c[3].metric("Median AE (dec)", M.get("MedianAE_decades", "?"))
    # full table
    def fmt(x):
        return f"{x:.3e}" if isinstance(x, float) and abs(x) < 1e-2 and x != 0 else f"{x}"
    rows = [
        ("R² (signed-log)", M.get("R2"), "variance explained; ≥0.90 excellent"),
        ("Adjusted R²", M.get("Adjusted_R2"), "≈ R² → no over-fitting"),
        ("Explained variance", M.get("Explained_Variance"), "≥0.90 strong"),
        ("MAE (decades)", M.get("MAE_decades"), "average error"),
        ("Median abs. error (dec)", M.get("MedianAE_decades"), "≈10% error, typical point"),
        ("RMSE (decades)", M.get("RMSE_decades"), "inflated by noise-floor points"),
        ("Max error (decades)", M.get("MaxError_decades"), "one leakage-floor point"),
        ("R² (real current)", M.get("R2_current"), "fit in Amps"),
        ("RMSE (real current, A)", M.get("RMSE_current_A"), "error in Amps"),
        ("MAE (real current, A)", M.get("MAE_current_A"), "error in Amps"),
        ("Within 3× of TCAD", f'{M.get("within_3x_pct","?")}%', "points within factor 3"),
        ("Within 1.8× of TCAD", f'{M.get("within_1p8x_pct","?")}%', "points within factor 1.8"),
    ]
    st.dataframe(
        pd.DataFrame([(nm, fmt(v) if isinstance(v, float) else v, note)
                      for nm, v, note in rows],
                     columns=["Metric", "Value", "Reading"]),
        use_container_width=True, hide_index=True)
    st.caption("Note: RMSE and Max error look large only because a few reverse-bias "
               "points sit at the 1e-16 A leakage floor, where a physically tiny error "
               "counts as many decades. The median error and within-3× rate are the "
               "fair summary. R² ≥ 0.90 and within-3× ≥ 85% indicate strong agreement.")
    st.markdown("---")
